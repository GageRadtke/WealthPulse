...
public class AssetManagementService {
    ...
    public Asset updateQuantityOrCreate(User user, Asset incomingAsset) {
        ...
        Asset merged = mergeExistingPosition(user, incomingAsset);
        return merged != null ? merged : createAsset(user, incomingAsset);
    }

    private Asset mergeExistingPosition(User user, Asset incomingAsset) {
        if (incomingAsset instanceof StockAsset stock) {
            return mergeStock(user, stock);
        }
        if (incomingAsset instanceof MetalAsset metal) {
            return mergeMetal(user, metal);
        }
        return null;
    }
    ...
    public void deleteAsset(User user, Long id) {
        ...
        recordSale(user, asset, "Position removed from holdings");
    ...
    private Asset adjustExistingQuantity(User user, Asset incoming) {
        ...
        rejectOversell(currentQuantity, requestedDelta);
        double transactionPrice = positive(incoming.getPrice()) ? incoming.getPrice() : value(existing.getPrice());

        recordQuantityAdjustment(user, existing, requestedDelta, transactionPrice);

        existing.setQuantity(currentQuantity + requestedDelta);
        existing.setAmountPaid(calculateAdjustedCostBasis(
                value(existing.getAmountPaid()), currentQuantity, requestedDelta, transactionPrice));
    ...
    static double calculateAdjustedCostBasis(
            double amountPaid, double currentQuantity, double delta, double price) {
        if (delta > 0) {
            return amountPaid + delta * price;
        }
        if (delta < 0 && currentQuantity > 0) {
            return amountPaid * (currentQuantity + delta) / currentQuantity;
        }
        return amountPaid;
    }

    static void rejectOversell(double currentQuantity, double requestedDelta) {
        if (requestedDelta < -currentQuantity) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Cannot sell more than the owned quantity");
        }
    }
    ...
    private void recordQuantityAdjustment(User user, Asset existing, double requestedDelta, double transactionPrice) {
        ledgerService.record(user, existing,
                requestedDelta < 0 ? TransactionType.SELL : TransactionType.BUY,
                BigDecimal.valueOf(Math.abs(requestedDelta)), BigDecimal.valueOf(transactionPrice),
                BigDecimal.valueOf(Math.abs(requestedDelta) * transactionPrice),
                LocalDateTime.now(), "Quantity adjustment");
    }

    private void recordSale(User user, Asset asset, String note) {
    ...
    private void setLastUpdatedIfMissing(Asset asset) {
        if (asset.getLastUpdated() == null) {
            asset.setLastUpdated(LocalDateTime.now());
        }
    }

    private void propagateMetalPriceIfNeeded(User user, Asset asset) {
        if (asset instanceof MetalAsset metal) {
